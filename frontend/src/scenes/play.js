...
let gemArray = Koji.config.tiles.tiles;
...
export default class Play extends Phaser.Scene {
  ...
  create() {
    ...
    this.match2 = new Match2({
      columns: 7,
      rows: 8,
      items: Koji.config.tiles.tiles.length
    });
  ...
  addTile(col, row) {
    ...
    let gemIndex = Math.floor(Math.random() * gemArray.length);
    let spriteToAdd = gemArray[gemIndex];
  ...
  areClusters() {
    ...
    for (let i = 0; i < tilesVisible; i++) {
      for (let j = 0; j < this.match2.rows; j++) {
        let currentTile = this.match2.getTile(i, j);
